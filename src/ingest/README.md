...
- Fences are found at any nesting depth: inside list items, blockquotes, footnote definitions, and combinations of these (e.g. a negative example inside a blockquote inside a list item), not only directly under a heading. The "immediately following sibling must be the `yaml expect` block" rule applies within whichever container the fence sits in, so an expect block placed outside that container is correctly treated as an orphan rather than silently attached to the wrong example. The example's provenance (`source.line`) is always its own fence's line, not the enclosing list/blockquote's line.
