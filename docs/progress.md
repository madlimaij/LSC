...
| WP-07 | Report + review CLI | report-builder | WP-05 | G2 round (D27) done; awaiting targeted re-review |
...
### WP-07 review round 1 fixes and D25 additions (`report-builder`, 2026-09-26)

**What was built:**

A. Reviewer round-1 findings:
1. **Reviewed matches vs. the lines a match spans.** `src/report/match-span.ts` (new): `computeMatchSpan(rule, maskingConfig, fileText, line, column)` re-runs the rule's own pattern (via `matchRule`, `prepareFile`, `exactToRegex` — import-only from `src/engines`/`src/contract`, nothing there changed) to find the match's line span (start..end, inclusive — a `multiline: true` rule can match text continued onto a later line, db-read's trap T8) and every match of the same rule whose start line falls in that span. `src/report/review-session.ts`'s `buildReviewEntry` now takes the `Rule`, masking config and raw file text, uses the span for `code` instead of `matchedLineText` (kept, now only used as a helper), lists every same-rule match on the spanned lines in `expected` for `verdict: correct` (sorted by line then capture), and **refuses** (`{ ok: false, reason }`) a `false_positive` when any *other* match of the same rule falls on those lines (D16 h: a negative example must contain no match of any construct — such a case cannot be written as a negative at all). `runReviewSession` gained a required `context: { ruleSet, readSampleFile }` parameter to supply these; a refusal or an unreadable sample file is reported through the existing `onSkipped` callback. `lsc review` now requires `--ruleset <file>` and `--sample <dir>` (in addition to `--skills-dir`/`--reviews-file`) to supply this context; `src/cli/commands/review.ts` reads sample files straight from `--sample <dir>` by `SampleMatch.file`. Added the reviewer's exact reproduction as an end-to-end test (`tests/report/review-session.test.ts` "the reviewer's db-read reproduction"): builds `reviews.yaml` entries for all three sample matches on `orders/order_lines.tl` (the multiline `READ &` match and the two same-line matches), loads them back with `parseReviews`, runs `db-read` against them with `matchRule`/`diffExample`, and asserts 0 failures (was 3 under the old single-line `code`). No runner change was needed or requested.
2. **Tests that didn't prove their claim, fixed as directed:**
   - `tests/report/markdown.test.ts` / `html.test.ts`: a new `brokenRuleSetWithAllThreeDefects()` helper (`tests/report/helpers.ts`) applies all three WP-05 breakage recipes at once (db-read forced per-line, a loosened db-write pattern, swapped proc-definition captures). New `markdownRuleSection`/`htmlRuleSection` helpers isolate one rule's own rendered section; the rewritten tests assert each defect's specifics (missed line/captures, both unexpected `db-write` captures, both wrong-capture values) *inside that rule's own section*, and that every unaffected rule's section still says "OK".
   - `tests/report/html.test.ts` "escapes example text": now builds a report from the real Rule Set + examples + sample files (no fabricated snippet needed — real source already contains `<`, `>`, `&` and quotes: `billing.tl`'s `IF ROWCOUNT() > 0 THEN`, `order_lines.tl`'s `READ &` continuation, and quoted `FLAG("...")` literals) and asserts the escaped forms are present and the raw forms are not.
3. **Section order.** `tests/report/markdown.test.ts` / `html.test.ts` ("layout (D25 item 3)") now assert explicitly that a rule's own unreviewed sample matches and provenance sit inside that rule's section, before the global Provenance section — the layout the owner accepted as a recorded deviation. `src/report/README.md` states the full section order and the reasoning for the deviation.
4. **Coverage confidence needs a reason.** Every coverage row now links to (or, for `no-rule` rows, names) the place giving its confidence reason: rule sections gained an anchor (`<a id="rule-<id>">` in Markdown, `id="rule-<id>"` on the HTML `<section>`), and the Coverage table gained a "Confidence reason" column pointing there (`see \`db-read\` below` / `see <a href="#rule-db-read">...</a> below`).

B. Owner additions (D25 items 2 and 4), extending the report model and both renderers:
- **Lexical settings.** `Report.lexical` (model.ts), built by `buildReport` from `ruleSet.fileMatchers`/`lineComment`/`blockComment`/`stringDelimiters` when `--ruleset` is given; a new `## Lexical settings` section in both renderers.
- **Synthesis outcomes.** `src/report/synthesis-view.ts` (new): `buildSynthesisView(synthesis)` turns a `synthesis.json` (WP-09, `src/synth/synthesis-schema.ts`, **import only**) into `Report.synthesis` — each construct's status (validated/rejected/not-justified/skipped/not-attempted), reason, attempt count and per-attempt outcome/problems, plus the top-level token usage. `redactReviewProblem` withholds a failing review example's *captured repository-sample text*: `describeFailure` (`src/synth/construct-loop.ts`, read to confirm the exact format) puts real captures (table/callee names, …) into `problems` and tags review-sourced failures `[review example]`; the report keeps the example id, role and that tag, and drops everything after it. `lsc report` gained `--synthesis <file>` (Zod-validated against `SynthesisReportSchema`); a new `## Synthesis` section renders it.
- **Provider/model/recording origin.** `synthesis.json` (`src/synth/synthesis-schema.ts` as it stands after WP-09) has **no `provider`, `model` or recording-origin field** — only per-attempt and total token usage (`usage: { inputTokens, outputTokens }` / `{ ..., calls }`). The report shows the usage it does have and states this gap explicitly rather than omitting the row silently: `SynthesisView.providerNote` ("synthesis.json ... does not record provider, model or recording origin as of WP-09 ..."), rendered in both formats. This is the "if it doesn't, say exactly what's missing" case from the brief.
- **Unreviewed sample match count in the verdict line.** `OverallVerdict.unreviewedSampleMatchCount` (sum of every rule's `sampleMatches.length`); the verdict summary now reads e.g. "VALIDATED — 100% of own examples pass (74/74); 138 sample matches not yet reviewed; ...", and the verdict box/section lists it as its own bullet too.
- **`--skills-dir` hash-drift warning.** `src/report/skill-hash-check.ts` (new): `findSkillHashMismatches(ruleSetSourceSkills, currentSourceSkills)` / `describeSkillHashMismatch`. `lsc report` computes this (needs both `--ruleset` and `--skills-dir`) and warns on stderr (`WARNING: ...`) *and* in the rendered report's Provenance section (`Report.skillHashMismatches`).
- **`renderReportFiles`.** `src/report/render-files.ts` (new, the one other file-I/O point besides `load-results.ts`'s read): `renderReportFiles({ results, ruleSet?, examples?, synthesis?, outDir, baseName? }) -> { report, markdownPath, htmlPath }` builds the model and writes `<baseName>.md`/`<baseName>.html` (default `report`) to `outDir`, creating it if needed. Documented in `src/report/README.md` for llm-integrator to call from `lsc compile` next (D25 item 2: "`lsc compile` writes the report itself ... into `--out`"); this WP did not touch `src/cli/commands/compile.ts` (outside its owned folders).

**New/changed files:** `src/report/match-span.ts`, `synthesis-view.ts`, `skill-hash-check.ts`, `render-files.ts` (new); `model.ts`, `build.ts`, `markdown.ts`, `html.ts`, `review-session.ts`, `index.ts`, `README.md` (changed); `src/cli/commands/report.ts` (`--synthesis`, skill-hash warning), `review.ts` (`--ruleset`, `--sample`, span-aware `buildReviewEntry`) (changed). Tests: `tests/report/match-span.test.ts`, `synthesis-view.test.ts`, `skill-hash-check.test.ts`, `render-files.test.ts` (new); `helpers.ts`, `build.test.ts`, `markdown.test.ts`, `html.test.ts`, `review-session.test.ts`, `cli-report.test.ts`, `cli-review.test.ts`, `__snapshots__/markdown.test.ts.snap` (changed).

**Acceptance criteria (this round):**
- Reviewer finding 1 fixed with the exact reproduction as a test, 0 failures: **met** (`tests/report/review-session.test.ts` "the reviewer's db-read reproduction").
- Markdown/HTML "broken rule set" tests render all three breakages and check each inside its own rule section: **met** (`markdown.test.ts`, `html.test.ts`).
- "Escapes example text" uses a report with real snippet text containing `<`, `>`, `&` and quotes: **met** (`html.test.ts`).
- Section-order test for the D25 item 3 layout, stated in the README: **met** (`markdown.test.ts`/`html.test.ts` "layout (D25 item 3)"; `src/report/README.md`).
- Every coverage row has a reason or a pointer to one: **met** (`confidenceReasonPointer` in both renderers; anchors on rule sections).
- Lexical settings, synthesis outcomes (with review-example redaction), the unreviewed-sample-match count, and the skill-hash-drift warning are all shown, with an explicit statement of what synthesis.json is missing (provider/model/origin): **met** (`build.test.ts` "D25 owner additions", `synthesis-view.test.ts`, `skill-hash-check.test.ts`, `cli-report.test.ts`).
- `renderReportFiles` exposed and documented for `lsc compile` to call: **met** (`render-files.test.ts`; `src/report/README.md`).
- `npm run typecheck`, `npm run lint`, `npm test`: **met** — 63 files, 612 tests, clean typecheck and lint (this session).

**Deviations:**
1. `lsc review` now *requires* `--ruleset` and `--sample` (previously only `--skills-dir`/`--reviews-file`). Without the actual rule pattern and sample file text, a span-aware `buildReviewEntry` cannot be computed at all, so there is no safe fallback to the old single-line behaviour (that behaviour is exactly what reviewer finding 1 reported as wrong).
2. While fixing the tests for finding 4, I found that the frozen fixture Rule Set (`contract/fixtures/toylang.ruleset.json`, contract-architect-owned) stores `sourceSkills[].path` as `skills/module.md`, etc., but `contract/CONTRACT.md` §"sourceSkills" defines `path` as "relative to the Skill directory" (i.e. relative to `--skills-dir` itself, which `ingestSkills` produces as `module.md`, no prefix). Against the real fixture as shipped, `lsc report --ruleset <fixture> --skills-dir fixtures/toylang/skills` currently warns about every Skill file, always, even with nothing changed — not because anything drifted, but because of this path-convention mismatch. I did not touch the frozen fixture (outside this WP's ownership); `tests/report/cli-report.test.ts`'s two hash-check tests build a corrected Rule Set (real `ingestSkills(...).sourceSkills`) so the feature itself is tested against the documented convention. Flagged below for the owner/contract-architect.
3. `SynthesisReport` is imported directly from `src/synth/synthesis-schema.js` in `build.ts`/`synthesis-view.ts`/`render-files.ts` (types only), and `SynthesisReportSchema` from `src/synth/index.js` in the CLI (matching `lsc compile`'s existing import path) — both import-only, `src/synth/` untouched.
4. Kept `matchedLineText` exported from `review-session.ts` (now only used internally as a fallback query, and still directly tested) rather than removing it, since it is a small, harmless, separately-testable helper and removing it would be unrelated churn.
...
### WP-07 G2 round (D27) (`report-builder`, 2026-09-26)

**What was built:** Fixed the five remaining wave-3-round-2 defects and added the six D27 report changes, all inside `src/report/`, `src/cli/commands/report.ts`, `src/cli/commands/review.ts`, `tests/report/` (one disclosed one-line exception below).

A. Defects:
1. **`src/report/match-span.ts`'s `computeMatchSpan`**: the span now extends iteratively to cover the full extent (own end line) of every same-rule match that *starts* inside the current span, not just matches whose start line falls in the *original* match's own span — repeating until nothing changes. Reproduction: `fixtures/toylang/sample-repo/customers/messages.tl:4` (`READ inbox WHERE owner = uid AND NOT read`) has a second, case-insensitive db-read match on the trailing "read" that continues across the newline to capture `LET` on line 5; reviewing the first match (table=inbox) as `correct` now extends the span to lines 4-5 so both matches' captures survive a replay. New tests: `match-span.test.ts` "extends the span over a same-rule match that starts inside it but continues past it"; `review-session.test.ts`'s new `describe` block reproduces the exact scenario and adds the requested acceptance test — records `correct` for every one of the fixture Rule Set's 132 sample matches, round-trips each through `stringifyReviews`/`parseReviews`, and replays it against its own rule: 0 failures (`failures` array asserted empty), where the reviewer previously found 131/132 passing.
2. **`src/report/review-session.ts`'s `buildReviewEntry`**: a `false_positive` refusal now also runs *every other rule in the Rule Set* against the spanned code (`findOtherRuleMatch`, via `matchRule`/`prepareFile`) and refuses, naming the other rule, if any of them matches — not only the same rule as before (D16 h, D19 a: a negative example must contain no match of *any* construct). The function's `maskingConfig: CommentStringConfig` parameter is now typed `ruleSet: RuleSet` (every caller already passed a full `RuleSet`; this only widens what the function is allowed to read from it, no behavioural change to existing callers). New tests reproduce the brief's exact example (`MODULE demo / PROC p() / READ t WHERE a = b; CALL helper() / ENDPROC`; a `false_positive` on the db-read match at 3:3 is refused, naming `call-statement`) and confirm `correct` is unaffected on the same match.
3. **`tests/report/html.test.ts`**: added an assertion using real content containing `<` (`fixtures/toylang/sample-repo/inventory/stock.tl:5`, `qty < reorder_level` → `qty &lt; reorder_level`), and verified by mutation — temporarily removing `esc()`'s `<` replacement in `src/report/html.ts` now fails this test (confirmed, then reverted).
4. **`src/report/skill-hash-check.ts`**: new `findNewSkillFiles`/`describeNewSkillFile` — every Skill file present under a fresh `--skills-dir` hash but absent from the Rule Set's `sourceSkills` (added after compile). Wired into `Report.newSkillFiles` (`build.ts`), both renderers, and `lsc report`'s stderr warnings (`src/cli/commands/report.ts`), the same way the existing "hash differs"/"removed" cases already were. New unit tests (`skill-hash-check.test.ts`) and an end-to-end CLI test (`cli-report.test.ts`).
5. **"No sample scanned" vs. "no unreviewed matches"**: `OverallVerdict` gained `sampleScanned`/`filesScannedCount` (`results.filesScanned.length`); the verdict summary sentence and both renderers' verdict bullets now say "no sample repository scanned" (not "no unreviewed sample matches") whenever no sample scan ran at all, everywhere the count appeared. Combined with item B.b below. New tests in `build.test.ts` distinguish the two cases explicitly.

B. Report changes (D27 a–f):
a. `renderVerdict`/verdict-box now show a "not produced by a real model" statement right next to the verdict (`modelSourceWarningNearVerdict[Html]`, reading `report.synthesis?.modelSource`), in both formats, in addition to the existing Synthesis-section statement.
b. A new, always-shown verdict bullet: "**Sample coverage shows matches only**: it cannot show constructs the rules missed in the sample" — worded differently depending on whether a sample was scanned at all (ties into A5).
c. A compact "Unreviewed sample matches per rule" table (rule id linking to `#rule-<id>`, type, count) placed after Coverage, before Lexical settings/Rules, in both formats; the inline per-rule listing is unchanged. In HTML, a rule with more than 5 sample matches now collapses its list behind `<details><summary>`.
d. A rejected rule's coverage row and its own section heading now say `rejected (dropped at export)` (in addition to the existing "DEFECT" badge/class, kept so earlier tests asserting on "DEFECT" still hold). A `ruleSetVersion === '0.0.0-draft'` now triggers a verdict-area warning: "Draft Rule Set: must not be delivered to Navigator", in both formats.
e. `SynthesisView` gained `lexicalAttempts` (`synthesis.lexical.attempts`, built by `synthesis-view.ts`'s new `buildLexicalAttempts` — no redaction needed, no repository-sample text is ever part of a lexical proposal). Both renderers show it in the `## Lexical settings` section, right after the accepted settings, as "attempt N: **outcome** — proposed: ... — refused because: ...". Verified against the real `wp09-reject` recording: attempt 1 is refused with `lineComment: "//" does not appear in the documentation`, attempt 2 is accepted.
f. `explainConfidence`'s reason string starts with the level word (`"high — ..."`) by design (self-contained), but the rule-section confidence line already shows the level in `**level**`, so it read "high — high — ...". New `reasonWithoutLeadingLevel` (`confidence-reason.ts`) strips a matching leading `"<level> — "` for display only; `explainConfidence`'s own return value and its existing tests are unchanged.

**Regenerated `.lsc/g2/`:** ran `npm run build`, then `lsc compile fixtures/toylang/skills --sample fixtures/toylang/sample-repo --provider fake --recordings fixtures/recordings/wp09 --out .lsc/g2-run/healthy` and the same with `fixtures/recordings/wp09-reject` → `.lsc/g2-run/reject` (no `lsc.config.json` needed: the default configuration plus `--provider fake --recordings <dir>` is sufficient). Copied each run's `report.md`/`report.html` to `.lsc/g2/{healthy,reject}.report.{md,html}`. Spot-checked both: the reject report shows the draft warning, the "not produced by a real model" statement near the verdict, the `call` rule as `rejected (dropped at export)`, the per-rule sample-match table, and the lexical proposal history with the refused `//` attempt and its reason, exactly as the brief describes.

**Acceptance criteria (this round):**
- Every A1–A5 defect fixed with the exact reproduction (or, for A3, mutation-verified) as a test: **met** (`match-span.test.ts`, `review-session.test.ts`, `html.test.ts`, `skill-hash-check.test.ts`, `cli-report.test.ts`, `build.test.ts`).
- Every B a–f change present in both Markdown and HTML, exercised by a test: **met** (`build.test.ts`, `markdown.test.ts`/`html.test.ts` snapshot + new tests, `cli-report.test.ts`).
- Markdown snapshot re-recorded; diff limited to exactly these changes (confirmed by reading `git diff tests/report/__snapshots__/markdown.test.ts.snap`): **met**.
- `.lsc/g2/` regenerated from two real `lsc compile` runs (`--sample`, `--provider fake --recordings <dir>`), copied as `healthy.report.*`/`reject.report.*`: **met**.
- `npm run typecheck`, `npm run lint`, `npm test`: **met** — 64 files, 649 tests, clean typecheck and lint.

**Deviation (disclosed, one line outside owned folders):** `tests/synth/compile-cli.test.ts`'s "the report written by compile shows call as rejected..." test asserted the exact old wording `'no unreviewed sample matches'` for a scenario that runs without `--sample` — the precise case A5/B.b changes. Since this is the literal, mandated consequence of this round's fix, not an unrelated behavioural change, I updated that one assertion (`'no sample repository scanned'`) rather than leave the suite red; everything else in that file is untouched. Flagged here for the orchestrator/`llm-integrator` to confirm.

**Open questions:**
- Whether the coverage table's "rejected (dropped at export)" wording, and the rule-heading "DEFECT — rejected (dropped at export)" badge text, should eventually replace "DEFECT" outright rather than append to it (kept additive here to avoid touching wave-3 tests unrelated to this round).
- Carried over: should `--format json` ever become a stable contract? Review still has no prompt for editing captures.
- Whether `tests/synth/compile-cli.test.ts`'s one-line wording update (above) should instead have been left to `llm-integrator`/orchestrator to make in a separate pass.
