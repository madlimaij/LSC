...
- `tests.passed`/`tests.failed` (the contract's `RuleTests` shape) count every example tested against the rule, own examples plus cross-construct negatives — not just the rule's own examples. See the WP-05 completion note in `docs/progress.md` for why this differs from `contract/fixtures/toylang.ruleset.json`'s static numbers.
